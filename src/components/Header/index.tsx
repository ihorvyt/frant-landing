...
const Index = ({hide, setShowLang}: { hide: boolean, setShowLang: (b: boolean) => void }) => {
    ...
    return (
        <header
            className={`header ${hide || isVisible && !showBurger ? 'header--hide' : ''} ${showBurger ? 'header--show' : ''}`}
        >
            <Link to={'banner'} smooth={true}
                  duration={1000} onClick={() => setShowBurger(false)} className="logo">
            ...
            <button className={`burger-menu-icon ${showBurger ? 'active' : ''}`} onClick={() => setShowBurger(!showBurger)}>
                <svg width="68" height="32" viewBox="0 0 68 32" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <line x1="11" y1="2" x2="57" y2="2" stroke="#EEEAE4" strokeWidth="3"/>
                    <line x1="1.31134e-07" y1="16" x2="68" y2="16" stroke="#EEEAE4" strokeWidth="3"/>
